...
| File | Purpose | Required? |
|------|---------|-----------|
| `sw.js` | **Service worker.** Caches the app so it loads offline after the first visit, and powers "install to home screen". The version string inside (e.g. `indic-reader-v62`) is bumped on every update to force browsers to fetch the new version. | Optional (enables offline) |
