...
- **Inline script-switching OCR** — short embedded Sanskrit quotes in Gujarati prose often misread (limitation of all OCR engines, not specific to this app). Mitigated by [Re-OCR a region](#re-ocr-a-region) or **Fix a single line** with Sanskrit-only hints, and manual text/language correction.
